...
from kindynsyn.synthesizer.steps import (
    ...
    QuasiStaticExternalForcePropagationStep
)
...
from kindynsyn_tutorial.my_controller import EX_CTRL, \
    MyCartesianControllerStep, MyCartesianControllerTranslator
...
def solver_configurator(g, cache, ROB, slv_algo):
    ...
    ext_prop = QuasiStaticExternalForcePropagationStep(g, cache, slv_algo, dyn_coord, dyn, kc, kc_stat)
    ...
    out_1 = SweepConfig(
        ...
        steps=[index, j_mot, j_dyn, if_msr, pos_prop, pos_acc, vel_prop, acc_prop, my_ctrl])
    in_1 = SweepConfig(
        direction=SweepDirection.INWARD,
        steps=[rbi, f_nrt, nrt_prop, ext_prop])
    out_2 = SweepConfig(
        direction=SweepDirection.OUTWARD,
        steps=[my_slv, if_cmd])
